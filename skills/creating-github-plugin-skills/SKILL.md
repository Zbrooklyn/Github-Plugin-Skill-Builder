...
When the repository root is the plugin package, use:
...
Keep the marketplace entry's `source` set to `./` when the repository root is the plugin package. Keep plugin names consistent between the marketplace and plugin manifest.
...
- Plugin names agree across manifests.
...
For ChatGPT web compatibility, do not add MCP configuration merely to make a skills package count as a plugin. Imported plugins that declare MCP servers can be labeled Desktop only.
